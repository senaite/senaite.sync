# Contributing to senaite.sync

Third-party contributions are essential for keeping `senaite.sync` continuously 
...
The following is a set of guidelines for contributing to senaite.sync, which is 
...
bugs](https://github.com/senaite/senaite.sync/issues)? Do you have a
suggestion for improvement? Then by all means please [submit a new
issue](https://github.com/senaite/senaite.sync/issues/new), and do not
hesitate to comment on existing [open
issues](https://github.com/senaite/senaite.sync/issues).
...
The list of [open issues](https://github.com/senaite/senaite.sync/issues)
...
### Preliminaries
   ...
   2. [Fork senaite.sync from GitHub](
   https://github.com/senaite/senaite.sync/fork). This will copy the whole
   `senaite.sync` repository to your personal account.
   ...
   the actual username of your GitHub account):
          ...
          $ git clone https://github.com/YOUR_USERNAME/senaite.sync

   4. Your forked repository https://github.com/YOUR_USERNAME/senaite.sync will 
   receive the default name of `origin`. You can also add the original 
   `senaite.sync` repository, which is usually called `upstream`:

          $ cd senaite.sync
          $ git remote add upstream https://github.com/senaite/senaite.sync.git
...
Alternatively, you can merge `senaite.sync`'s `master` into your branch:
...
pros and cons between doing a `git rebase` or `git merge`. Good documentation on
...
request](https://github.com/senaite/senaite.sync/compare/). Head to your
 ...
 * Add a changelog entry in [CHANGES.rst](https://github.com/senaite/senaite.sync/CHANGES.rst)
